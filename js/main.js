...
function posicion(orden, flujo, inicio) {
    ...
    for (let i = inicio; i < flujo.length; i++) {
    ...
    if (cont != orden.length - 1) return -1;
...
function resultado(flujo) {
    ...
    while (cont < flujo.length) {
        ...
        if (orden.length < marcos) {
            ...
            if (!existe(orden, flujo[cont])) {
                let pos = posicion(orden, flujo, cont);
                let idx = indice(orden, aux[0]);
                if (pos != -1) {
                    orden[pos] = flujo[cont];
                } else {
                    orden[idx] = flujo[cont];
                    aux.splice(0, 1);
                    aux.push(flujo[cont]);
                }
